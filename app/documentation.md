...
* **Adjustment to p-values** -- Whether and how to treat p-values, given the fact that every federation constitutes an independent test and thus we could have problems arising from multiple comparisons. The options are: (i) "None", which reports the p-values as described below; (ii) "False discovery rate" which adjusts the p-values using the method of Benjamini and Hochberg[^1]; (iii) "Holm", which adjusts the p-values using Holm's method[^2]. The unadjusted p-values for every federation are obtained as follows: we first generate one million permutation samples of the ratings, and calculate the fraction *f* of the gender differences based on these samples that are less than the true observed rating difference. We then convert this fraction into a two-sided p-value via the standard transformation *p* = 2 &times; min(*f*, 1 - *f*).
...
This follows right below the summary statistics, and gives a tabular summary of the data. Each row corresponds to a unique federation. The six columns are: federation (with three-letter abbreviations), women (the number of FIDE-rated women of the given federation), men (number of FIDE-rated men), rating gap (difference in the chosen rating statistic between men and women), permutation (mean plus/minus one standard deviation of the one million permutation null samples), and p-value (these are all empty when p-values are not applicable, otherwise they contain the unadjusted or adjusted p-values, depending on user choice). The rating gaps are shown to one, the p-values to four decimal precision. When p-values are less than 0.0001, they are displayed as < 10<sup>-4</sup>.
