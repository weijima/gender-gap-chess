...
ratings <- function(rating_data, jun, inact) {
  rating_data %>%
    filter(if (jun) TRUE else age >= 20) %>%
    filter(if (inact) TRUE else active)
}


nulls <- function(null_data = null_data, jun, inact, rating_floor = 1000L) {
  null_data %>%
    filter(if (jun) juniors else !juniors) %>%
    filter(if (inact) inactives else !inactives) %>%
    filter(floor == rating_floor)
}


feds <- function(null_data, jun, inact, rating_floor) {
  nulls(null_data, jun, inact, rating_floor) %>%
    pull(fed) %>%
    unique()
}
...
scatter_hist_figure <- function(result_data, rating_data, feds_to_keep,
                                age_experience_data) {
...
age_experience_data <- read_csv("data/age-experience-tab.csv", col_types = "cllicddddd")
...
crossing(
  ...
  mutate(plt = pmap(list(juniors, inactives, floor), \(jun, inact, fl) {
    scatter_hist_figure(
      result_data = nulls(null_data, jun, inact),
      rating_data = ratings(rating_data, jun, inact),
      feds_to_keep = feds(null_data, jun, inact, fl),
      age_experience_data = age_experience_data %>%
        filter(floor == fl & juniors == jun & inactives == inact) %>%
